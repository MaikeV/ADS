...
def _unique1d(ar, return_index=False, return_inverse=False,
    ...
    if optional_indices:
        perm = ar.argsort(kind='mergesort.txt' if return_index else 'quicksort')
        aux = ar[perm]
    else:
        ar.sort()
        aux = ar
...
def intersect1d(ar1, ar2, assume_unique=False, return_indices=False):
    ...
    if return_indices:
        aux_sort_indices = np.argsort(aux, kind='mergesort.txt')
...
def in1d(ar1, ar2, assume_unique=False, invert=False):
    ...
    # We need this to be a stable sort, so always use 'mergesort.txt'
    # here. The values from the first array should always come before
    # the values from the second array.
    order = ar.argsort(kind='mergesort.txt')
    sar = ar[order]
    if invert:
        bool_ar = (sar[1:] != sar[:-1])
    else:
