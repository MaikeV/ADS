...
TEST_CASE("List Testing", "[List]") {
    ...
    SECTION("Vertauschen von zwei Elementen und testen der Zeiger") {
        ...
        Node *tmp = get_anker(test_list);
        //pruefe ob 2 an neuem Platz
        for (int i = 0; i < 3; i++)
            tmp = tmp->next;
        int key = tmp->key;
        REQUIRE(key == 2);
        //Pruefe ob 7 an neuem Platz
        for (int i = 0; i < 3; i++)
            tmp = tmp->next;
        key = tmp->key;
        REQUIRE(key == 7);
        tmp = tmp->next;
        //Pruefe ob Anker wieder erreichbar
        for (int i = 0; i < 7; i++)
            tmp = tmp->prev;
        key = tmp->key;
        REQUIRE(key == 0);
        ...
        tmp = tmp->next;
        //Pruefe ob neuer erster richtig
        key = tmp->key;
        REQUIRE(key == 5);
        //Pruefe ob 4 an neuem Platz
        for (int i = 0; i < 3; i++)
            tmp = tmp->next;
        key = tmp->key;
        REQUIRE(key == 4);
        ...
        for (int i = 0; i < 5; i++)
            tmp = tmp->next;
        //Pruefe ob 1 an neuem Platz
        key = tmp->key;
        REQUIRE(key == 1);
        //Pruefe ob neuer letzter richtig
        for (int i = 0; i < 3; i++)
            tmp = tmp->next;
        key = tmp->key;
        REQUIRE(key == 3);
